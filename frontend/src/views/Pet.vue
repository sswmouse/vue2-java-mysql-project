...
export default {
    ...
    methods: {
         ...
         * 加载角色已穿戴的宠物
         */
        async loadEquippedPets() {
            try {
                const res = await this.$request({
                    url: petApi.getEquipped(this.selectedCharacterId),
                    method: 'get'
                })
                this.equippedPetIds = Array.isArray(res) ? res : (res.data || [])
            } catch (error) {
                console.error('加载已穿戴宠物失败:', error)
                this.equippedPetIds = []
            }
        },
        ...
        handleEdit(row) {
            ...
            this.form = {
                ...
                mpBonus: row.mpBonus || 0,
                attackStrength: row.attackStrength || 0,
                threeAttack: row.threeAttack || 0,
                whiteDamage: row.whiteDamage || 0,
                extraYellow: row.extraYellow || 0,
                cdReduction: row.cdReduction || 0,
                fame: row.fame || 0
            }
            this.dialogVisible = true
        },
        ...
        async handleSave() {
            ...
            try {
                if (this.isEdit) {
                    await this.$request({
                        url: petApi.update(this.editingId),
                        method: 'put',
                        data: this.form
                    })
        ...
        async equipPet(row) {
            ...
            try {
                await this.$request({
                    url: petApi.equip(row.id, this.selectedCharacterId),
                    method: 'post'
                })
                this.$message.success(`已穿戴宠物「${row.name}」`)
                this.loadEquippedPets()
            } catch (error) {
                console.error('穿戴宠物失败:', error)
                this.$message.error('穿戴宠物失败')
            }
        },
        ...
        async unequipPet(row) {
            ...
            try {
                await this.$request({
                    url: petApi.unequip(row.id, this.selectedCharacterId),
                    method: 'delete'
                })
                this.$message.success(`已卸下宠物「${row.name}」`)
                this.loadEquippedPets()
            } catch (error) {
                console.error('卸下宠物失败:', error)
                this.$message.error('卸下宠物失败')
            }
        }
    }
}
